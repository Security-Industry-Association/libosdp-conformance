---
title: OSDP Testing with libosdp-conformance
---
...
Each (most) command and response calls an action routine when it is processed.
These are stored in /opt/osdp-conformance/actions.  The default plugins log
to /opt/osdp-conformance/log/actions.log when a command or response is called.

# X.99 Action osdp_XRD #

This action routine is called with these parameters:

- PD address
- mode
- preply value
- hex string of details in response

# Appendix X. Saved Parameters #

File ___osdp-saved-parameters.json___ is used to save the last set Secure Channel
key and the current speed.

key - value is a 16 byte AES key in hex e.g.
\"aabbccdd11223344eeff001199887766\"

Delete this file if you want to reset to the defaults.
